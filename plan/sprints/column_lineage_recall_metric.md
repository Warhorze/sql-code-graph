...
**Status: REVIEWED** (plan-reviewer 2026-06-13: APPROVED-WITH-AMENDMENTS; amendments
A1–A4 folded into Steps 1.3/1.4 and Test Strategy; A5 (`uv lock`) was already present in
Step 1.6). Ready for implementation — ship as **v1.27.0**.
...
- File: [`coverage.py`](../../src/sqlcg/cli/coverage.py) — add
  ...
  Place the new `run_read_routed` call **adjacent to `q_cte_source_gap`** so the recall
  metrics stay grouped.
- **Amendment A1 (plan-review, REQUIRED)**: `tests/unit/test_coverage_metrics.py` drives
  `collect_coverage()` with `_make_coverage_side_effect(_CANNED_ROWS)`, which returns canned
  rows **by call index** — so adding a `run_read_routed` call shifts every later call
  (including the trailing inline `Repo` query) by one position. After wiring the new call,
  insert a matching `[{"resolvable_write_col_edges": <N>}]` entry into `_CANNED_ROWS` at the
  exact position the new call occupies (so the `Repo`-path entry shifts to follow it), and
  update the existing `test_coverage_metrics_collect_returns_populated_stats` assertion to
  verify `stats.resolvable_write_col_edges == <N>`. Without this the existing test silently
  reads the wrong canned row and `cte_source_gap_writes`/other fields go wrong. Verify the
  call order in the source after wiring — do not assume the index.
- Acceptance: `collect_coverage()` on a populated graph returns a `CoverageStats` whose
  `resolvable_write_col_edges` equals the direct SQL count; `test_coverage_metrics.py`
  passes with the shifted `_CANNED_ROWS`.

**Step 1.4**: Add the render line.
- File: [`coverage.py`](../../src/sqlcg/cli/coverage.py) `render_coverage_lines()`, **appended
  immediately after the `cte_source_gap_writes` render line (currently ~L718), before the
  identity-health block** (Amendment A2):
  ```python
  lines.append(
...
- **Unit — value correctness**: build a small in-memory DuckDB graph with (a) a write
  ...
  row, (d) a non-write (SELECT) query with edges, **(e) a write-kind query with
  `target_table = ''` (empty) that HAS a `SELECTS_FROM` row and edges** (Amendment A3 —
  pins the `target_table <> ''` discriminator behaviourally). Assert
  `resolvable_write_col_edges == 3` (only the edges from (a) are counted; (e) is excluded by
  the discriminator). Verifies the `SELECTS_FROM` gate, the kind filter, and the
  `target_table <> ''` discriminator each exclude the right rows.
...
- **Integration**: run `collect_coverage()` against a real indexed in-memory graph and
  assert the field is **`> 0` AND** equals an independent direct `COUNT(*)` of the same SQL
  (Amendment A4) — the `> 0` clause guards against the `.get(...) or 0` fallback silently
  returning 0 from a wrong/typo'd row key while the direct count is also 0 on a too-thin
  fixture; the fixture must therefore produce a non-zero count.
