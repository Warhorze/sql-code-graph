...
def test_schema_version_matches_current_migration():
    ...
    Originally pinned to '6' for the #35 PR-1 ExternalConsumer DDL; bumped 7 -> 8
    for ``File.skip_counts`` column added by
    plan/sprints/sprint_postmortem_fixes.md §PR 5 (persist col_lineage_skip:*
    reasons queryably). Update this literal in lockstep whenever schema.py bumps
    the constant.
    """
    from sqlcg.core.schema import SCHEMA_VERSION

    assert SCHEMA_VERSION == "8", (
        f"SCHEMA_VERSION must be '8' after the File.skip_counts bump; currently {SCHEMA_VERSION!r}"
    )
